...
public class Terrain : MonoBehaviour
{
    ...
    public void ModifyTerrain(MapPoint point, bool lower)
    {
        ...
        GameController.Instance.OnStructureTerrainDisturbed?.Invoke();
    ...
    /// </summary>
    /// <param name="chunk">The index of the chunk the point is in.</param>
    /// <param name="tile">The coordinates of the tile.</param>
    /// <param name="isOccupied">True if the tile is occupied by some structure, false otherwise.</param>
    public void SetOccupiedTile((int x, int z) tile, bool isOccupied)
    {
        TerrainChunk chunk = GetChunkByIndex(GetChunkIndex(tile));

        if (!chunk.IsTileOccupied(tile.x, tile.z))
            chunk.SetOccupiedTile(tile.x, tile.z, isOccupied);
    }
