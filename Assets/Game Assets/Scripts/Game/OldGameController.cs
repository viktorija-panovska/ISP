...
public class OldGameController : NetworkBehaviour
{
    public static OldGameController Instance { get; private set; }
    ...
    private void SpawnStarterUnits()
    {
        ...
        for (int i = 0; i <= 1; ++i)
        {
            ...
            for (int j = 0; j < STARTING_UNITS; ++j)
            {
                ...
                SpawnUnit(new BaseUnit(), i == 0 ? Team.RED : Team.BLUE, location, isLeader: j == leader);
    ...
    {
        OldPlayerController.Instance.SetupCameraController(cameraStart, GetNetworkObject(viewId).gameObject);
    }
    ...
    public void SpawnUnit(IUnitType unitType, Team team, WorldLocation spawnLocation, House originHouse = null, bool newUnit = true, bool isLeader = false)
    {
    ...
    public bool HasLeader(Team team) => leaders[(int)team - 1] != null;

    public bool HasKnight(Team team) => knights[(int)team - 1].Count > 0;

    public bool AreMaxUnitsReached(Team teams) => units[(int)teams - 1] >= MAX_UNITS_PER_PLAYER;
    ...
    public void SpawnHouse(List<WorldLocation> vertices, Team team = Team.NONE, bool isDestroyedHouse = false)
    {
    ...
    [ServerRpc(RequireOwnership = false)]
    public void MoveUnitsServerRpc(Team team, WorldLocation endLocation)
    {
    ...
    private void SpawnFlag(Team team, Vector3 location)
    {
        int index = (int)team - 1;
    ...
    public void EndFollowForAll(Team team)
    {
        int index = (int)team - 1;
    ...
    private void ReleaseUnitsFromAllHouses(Team team)
    {
    ...
    [ServerRpc(RequireOwnership = false)]
    public void SendKnightServerRpc(Team team)
    {
        int index = (int)team - 1;
    ...
    {
        Time.timeScale = 0f;
        EndGameClientRpc(loser == Team.RED ? Team.BLUE : Team.RED);
    }

    [ClientRpc]
    private void EndGameClientRpc(Team winner)
    {
        OldPlayerController.Instance.EndGame(winner);
    }
    ...
    {
        OldPlayerController.Instance.RestartGame(cameraStart);
    }
