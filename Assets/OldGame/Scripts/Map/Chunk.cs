...
public class Chunk
{
    ...
    public bool IsSpaceUnderwater(int x, int z)
    {
        ...
        return GetVertexHeightAtIndex(index.x, index.z) <= OldGameController.Instance.WaterLevel.Value;
    }
    ...
    public void DestroyUnderwaterFormations()
    {
        ...
        foreach ((int x, int z) key in formations.Keys)
            if (GetVertexHeightAtIndex(key.x, key.z) <= OldGameController.Instance.WaterLevel.Value)
                underwater.Add(key);

        foreach ((int x, int z) in underwater)
        {
            OldGameController.Instance.DestroyFormation(formations[(x, z)].gameObject);
    ...
    {
        int height = 0;//Mathf.FloorToInt(PerlinNoiseGenerator.GetPerlinAtPosition(ChunkPosition + meshData.vertices[vertexIndex]) * MAX_STEPS) * STEP_HEIGHT;
    ...
    private void UpdateHeightAtPoint(int x, int z, bool decrease, ref HashSet<(int, int)> modifiedVertices)
    {
        ...
        if (formations.ContainsKey((x, z)))
        {
            if (formations[(x, z)].ShouldDestroy())
            {
                ...
                OldGameController.Instance.MoveFormation(GetVertexHeightAtIndex(x, z), formations[(x, z)].gameObject);
        }
